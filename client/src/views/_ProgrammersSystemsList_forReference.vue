...
<!-- ************************************************************************************************* -->
      <v-col cols="5">
        <v-scroll-y-transition>
          <v-card v-if="blnMounted" :disabled="blnLoading" class="MyCard1 mx-2" tile>
            ...
            if(ProjectDetails.Gitlab_ID != null){
              ...
              axios.get(MembersAPI).then(Members => {
                if(Members.data.length > 0){
                  const checkIfInclude = Members.data.map(MembersDetails => {return MembersDetails.name}).includes(this.Programmer)
                  if(checkIfInclude){
